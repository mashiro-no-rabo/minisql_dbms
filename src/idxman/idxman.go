...
func (self *node) splitNode () (KeyType, *node) {
    ...
    if self.isLeaf() {
        ...
        k = self.keys[order / 2]
        self.keys = self.keys[:order / 2]
    } else {
        n.keys = append(n.keys, self.keys[order / 2 : ]...)
        n.children = append(n.children, self.children[order / 2 : ]...)
        
        k = self.keys[order / 2 - 1]
        self.keys = self.keys[:order / 2 - 1]
        self.children = self.children[:order / 2]
    }
    self.keyCnt = order / 2
    return k, n
}
...
func (self *idxMan) Print() {
    ...
    for {
        ...
        fmt.Print(n, "\t", n.parent, "\t")
        for _, key := range n.keys {
            fmt.Print(key.GetValue().ToString(), " ")
        }
        fmt.Println()
        if ! n.isLeaf() {
